...
@register.inclusion_tag('tags/side_menu_children.html', takes_context=True)
def side_menu_children(context, parent):
...
#for standard index listing
@register.inclusion_tag(
    'tags/news_index_listing.html',
    takes_context=True
)
def news_index_listing(context, calling_page):
    news = calling_page.get_children().filter(live=True)
    return {
        'news': news,
        # required by the pageurl tag that we want to use within this template
        'request': context['request'],
...
# News feed for home page
@register.inclusion_tag(
    'tags/news_listing_homepage.html',
    takes_context=True
)
def news_listing_homepage(context, count=2):
    news = NewPage.objects.filter(live=True).order_by('-date')[:10]

    #for new in news:
    #    carousel_item = new.carousel_items
